...
import { Component, Output, EventEmitter, OnInit, OnDestroy, Input, OnChanges, SimpleChanges, ViewChild } from '@angular/core';
import { FormsModule, ReactiveFormsModule } from '@angular/forms';
import { HttpClientModule } from '@angular/common/http';
import { DxButtonModule, DxPopupModule, DxScrollViewModule, DxTextBoxModule, DxSelectBoxModule, DxDataGridModule, DxDateBoxModule, DxDataGridComponent } from 'devextreme-angular';
...
export class AllocationFormComponent implements OnInit, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['initialAllocationData'] && this.initialAllocationData) {
      ...
      if (Array.isArray(this.initialAllocationData) && this.initialAllocationData.length) {
  ...
  this.expectedTotalNotional = Number(incoming?.notional ?? incoming?.Notional ?? 0);
  ...
  private applyInitialAllocationData(rows: any[]): void {
    ...
    if (this.initialAllocationData && this.initialAllocationData.length) {
      this.expectedTotalNotional = Number(this.initialAllocationData[0].notional ?? this.initialAllocationData[0].Notional ?? 0);
    }
    this.computeAllocationState();
  }
  ...
  loadFundCombinations(valuationDate?: string): void {
    ...
    const valDate = valuationDate ?? this.valuationDate ?? this.formatDateForApi(new Date());
    this.subscription.add(
      this.httpService.getFundCombination(valDate).subscribe({
        next: (response: any) => {
          if (response?.Success && response?.Model) {
            this.fundCombinations = response.Model.FundCombination || [];
  ...
  setValuationDate(date: Date | string | null): void {
    ...
    const formatted = (date instanceof Date) ? this.formatDateForApi(date) : this.formatDateForApi(new Date(date));
  ...
  isFormValid(): boolean {
  return this.allocationData && this.allocationData.length > 0 && this.allocationComplete;
  }
  ...
  loadTemplate(): void {
    ...
    this.subscription.add(
      this.httpService.getFundCombination(valDate).subscribe({
        next: (response: any) => {
          this.isLoadingTemplates = false;
          const allocations = response?.Model?.FundAllocation ?? response?.FundAllocation ?? [];

          // Filter allocations where Title matches the template's Title
          const matched = allocations.filter((a: any) => a.Title === title);
          ...
          const incomingNotional = (this.initialAllocationData && this.initialAllocationData.length) ?
            (this.initialAllocationData[0].notional ?? this.initialAllocationData[0].Notional ?? 0) : 0;

          // Build allocation rows by mapping percentage to notional amount and upfront
          const incomingUpfront = (this.initialAllocationData && this.initialAllocationData.length) ?
            (this.initialAllocationData[0].upfront ?? this.initialAllocationData[0].Upfront ?? 0) : 0;
          ...
          const sumNotional = rows.reduce((s: number, r: any) => s + (r.notional ?? 0), 0);
          const notionalDiff = +(incomingNotional - sumNotional).toFixed(2);
          if (Math.abs(notionalDiff) >= 0.01 && rows.length > 0) {
            // adjust last row's notional to absorb rounding diff
            rows[rows.length - 1].notional = +(rows[rows.length - 1].notional + notionalDiff).toFixed(2);
            // also adjust receive/pay accordingly
            rows[rows.length - 1].receiveNotional = rows[rows.length - 1].notional;
            rows[rows.length - 1].payNotional = rows[rows.length - 1].notional;
          }

          const sumUpfront = rows.reduce((s: number, r: any) => s + (r.upfront ?? 0), 0);
          const upfrontDiff = +(incomingUpfront - sumUpfront).toFixed(4);
          if (Math.abs(upfrontDiff) >= 0.0001 && rows.length > 0) {
            rows[rows.length - 1].upfront = +(rows[rows.length - 1].upfront + upfrontDiff).toFixed(4);
          }
        ...
        error: (err: any) => {
          this.isLoadingTemplates = false;
          console.error('Error loading allocations for template', err);
        }
      })
    );
  }
  ...
  public addNewRow(): void {
    try {
      const row = { account: '', notional: 0, notionalPercent: 0, receiveNotional: 0, payNotional: 0, upfront: 0 };
      this.allocationData = [...this.allocationData, row];
      // refresh grid if available
      try { this.allocGrid.instance.refresh(); } catch (e) {}
      this.computeAllocationState();
    } catch (e) {
  ...
  public deleteSelectedRows(): void {
    try {
      ...
      const selected = this.allocGrid.instance.getSelectedRowsData() || [];
      if (!selected.length) return;
      const remaining = this.allocationData.filter((r: any) => !selected.includes(r));
  ...
  public computeAllocationState(): void {
    try {
      const totalPercent = this.allocationData.reduce((s: number, r: any) => s + (Number(r.notionalPercent) || 0), 0);
      const sumReceiveNotional = this.allocationData.reduce((s: number, r: any) => s + (Number(r.receiveNotional) || 0), 0);
      ...
      if (this.expectedTotalNotional && this.expectedTotalNotional > 0) {
        ...
        notionalOk = Math.abs(sumReceiveNotional - this.expectedTotalNotional) <= 0.01;
      }
      ...
      this.allocationComplete = (totalPercent >= 99 && totalPercent <= 100) && notionalOk && allHaveAccountAndPct && this.allocationData.length > 0;
    } catch (e) {
  ...
  public onRowUpdated(e: any): void {
    try {
      ...
      const key = e.key;
      // find index
      const idx = this.allocationData.findIndex((r: any) => r === e.data || r.account === e.key || r === e.key);
      ...
      if (data.hasOwnProperty('receiveNotional') || data.hasOwnProperty('notional')) {
        // notional edited -> update percent and pay/upfront
        const newNotional = Number(data.receiveNotional ?? data.notional) || 0;
        // update corresponding entry by reference or by id
        const target = this.allocationData[idx] ?? this.allocationData.find((r: any) => r.account === (e.key?.account ?? e.key));
        if (target) {
          target.receiveNotional = Math.abs(newNotional);
          target.payNotional = Math.abs(this.NotionalCal(this.expectedTotalNotional || newNotional, (target.notionalPercent || 0)));
          // compute percent if expectedTotalNotional present
          if (this.expectedTotalNotional && this.expectedTotalNotional > 0) {
            target.notionalPercent = +(newNotional / this.expectedTotalNotional * 100);
          }
        }
      }
      ...
      if (data.hasOwnProperty('notionalPercent')) {
        ...
        const target = this.allocationData[idx] ?? this.allocationData.find((r: any) => r.account === (e.key?.account ?? e.key));
        if (target) {
          const total = this.expectedTotalNotional || this.allocationData.reduce((s: number, r: any) => s + (Number(r.receiveNotional) || 0), 0);
          const newNotional = +(total * (newPct / 100));
          target.receiveNotional = Math.abs(newNotional);
          target.payNotional = Math.abs(this.NotionalCal(total, newPct));
          target.upfront = this.NotionalCal(Number(this.initialAllocationData?.[0]?.upfront ?? 0), newPct);
  ...
  public deleteSingleRow(row: any): void {
    try {
      this.allocationData = this.allocationData.filter((r: any) => r !== row);
      try { this.allocGrid.instance.refresh(); } catch (e) {}
      this.computeAllocationState();
    } catch (e) {
