...
export class CDSStepperComponent implements OnInit, OnDestroy {
                ...
                try {
                    ...
                    if (tradeDateVal) {
                        if (typeof allocAny.setValuationDate === 'function') {
                            ...
                            allocAny.valuationDate =
                                tradeDateVal instanceof Date
                                    ? allocAny.formatDateForApi(tradeDateVal)
                                    : allocAny.formatDateForApi(
                                          new Date(tradeDateVal)
                                      );
                        }
                    }
                } catch (e) {
    ...
    onNextButtonClick(): void {
        ...
        if (this.selectedIndex === this.steps.length - 1) {
            ...
            const allocValid = this.allocationForm
                ? this.allocationForm.isFormValid()
                : false;
            if (!allocValid) {
                notify({
                    message:
                        'Please complete allocation until percentages sum to ~100% and each row has an account before finishing.',
                    type: 'error',
                    displayTime: 3000,
                    position: { my: 'center top', at: 'center top' },
                });
    ...
    isNextButtonDisabled(): boolean {
        if (this.selectedIndex === 0) {
            return this.cdsOptionsForm
                ? !this.cdsOptionsForm.tradeForm.valid
                : true;
        }
        ...
        const payload: CDSOptionModel[] = allocationData.map(
            (alloc: any, idx: number) => {
                const model: CDSOptionModel = {
                    ...
                    TradePrice: cdsData.TradePrice ?? null,
                    TraderRationale: cdsData.TraderRationale ?? '',
                    TradarAccount: cdsData.TradarAccount ?? null,
                    TradarPayAccount: cdsData.TradarPayAccount ?? null,
                    FinUpdateTime: new Date(),
                    CurrencyId: cdsData.CurrencyId ?? null,
                    Notional: alloc.notional ?? null,
                    EnteredById: null,
                    ...
                    IndeptAmt: cdsData.IndeptAmt ?? null,
                    IndeptCCY: cdsData.IndeptCCY ?? null,
                    OptionType: cdsData.OptionType ?? null,
                    OptionStyle: cdsData.OptionStyle ?? null,
                    ExchangeRate: cdsData.ExchangeRate ?? null,
                    ...
                    UtiId: cdsData.UtiId ?? null,
                    UtiPrefix: cdsData.UtiPrefix ?? null,
                    FixedRate: cdsData.FixedRate ?? null,
                };
                return model;
            }
        );
                ...
                next: (response) => {
                    ...
                    if (!response.error) {
                        notify({
                            message:
                                response.message ||
                                'CDS Option trade inserted successfully!',
                            type: 'success',
                            position: { my: 'center top', at: 'center top' },
                        });
                        ...
                        notify({
                            ...
                            message: `Error: ${
                                response.message || 'Failed to insert trade.'
                            }`,
                            type: 'error',
                            position: { my: 'center top', at: 'center top' },
                        });
